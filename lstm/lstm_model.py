...
class ProjEnoughMultiTokenPredictionLayer(MultiTokenPredictionLayer):
    def __init__(
        self,
        config: TransformerConfig,
        submodules: MultiTokenPredictionLayerSubmodules,
        ...
        if self.layer_number == 1:
            ...
            self.f_proj = build_module(
                self.submodules.eh_proj,
                self.config.hidden_size,
                self.config.hidden_size * NUM_PREDICTION_TOKENS_FOCUSED,
                config=self.config,
                init_method=self.config.init_method,
                gather_output=False,
                bias=False,
    ...
    def _concat_embeddings(self, hidden_states: torch.Tensor, decoder_input: torch.Tensor):
        if self.layer_number == 1:
            ...
            hidden_states = self.fnorm(hidden_states)
            hidden_states = make_viewless_tensor(inp=hidden_states, requires_grad=True, keep_graph=True)
            hidden_states, _ = self.f_proj(hidden_states)
            hidden_states = gather_from_tensor_model_parallel_region(hidden_states)

            hidden_states = hidden_states.view(s * b, -1, d).permute(1, 0, 2)
...
class ProjEnoughBlock(MultiTokenPredictionBlock):

    def _build_layers(self, model_comm_pgs):
        def build_layer(layer_spec, layer_number):
            layer_spec.module = ProjEnoughMultiTokenPredictionLayer
            return build_module(
                layer_spec,
                config=self.config,
                layer_number=layer_number,
    ...
    def forward(
        ...
        """
        attention_mask = torch.tril(
            torch.ones((NUM_PREDICTION_TOKENS, NUM_PREDICTION_TOKENS), device=attention_mask.device)
        ).reshape(1, 1, NUM_PREDICTION_TOKENS, NUM_PREDICTION_TOKENS )
        # output_str = f"{rotary_pos_emb.shape=}\n" if rotary_pos_emb is not None else "rotary_pos_emb is None\n"
        # output_str += f"{rotary_pos_cos.shape=}\n" if rotary_pos_cos is not None else "rotary_pos_cos is None\n"
        # output_str += f"{rotary_pos_sin.shape=}\n" if rotary_pos_sin is not None else "rotary_pos_sin is None\n"
        rotary_pos_emb = rotary_pos_emb[:NUM_PREDICTION_TOKENS]
        # print(output_str, end="", flush=True)
        # exit()
...
class ProjEnoughModel(GPTModel):
    def __init__(
        self,
        config: TransformerConfig,
        transformer_layer_spec: ModuleSpec,
    ...
    def _postprocess(
        ...
        if labels is None:
            ...
            for i in range(NUM_PREDICTION_TOKENS):
                ...
                if i < NUM_PREDICTION_TOKENS_FOCUSED:
                    loss_list.append(one_loss)
